...
export function Sidebar() {
  ...
  const renderNavItems = (items: typeof navItemsGroup1) => (
    items.map((item) => {
      ...
      return (
        <Link
          ...
          href={item.href}
          className={cn(
            "group relative flex items-center gap-3 rounded-2xl px-4 py-2.5 text-base font-bold transition-all duration-200",
            isActive 
              ? "text-slate-900" 
  ...
  return (
    <div className="hidden md:flex h-screen w-64 flex-col border-r-2 border-slate-200 bg-white">
      ...
      <nav className="flex-1 space-y-1.5 px-4 py-6 overflow-y-auto">
        <div className="text-xs font-bold text-slate-400 mb-2 mt-4 px-4">QUOTIDIEN</div>
        {renderNavItems(navItemsGroup1)}
        
        <div className="text-xs font-bold text-slate-400 mb-2 mt-8 px-4">PRÉPARATION & OUTILS</div>
        {renderNavItems(navItemsGroup2)}
      </nav>
